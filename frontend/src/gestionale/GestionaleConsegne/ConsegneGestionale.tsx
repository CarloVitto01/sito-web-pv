...
const DELIVERY_ENDPOINT = "/api/admin/config/delivery";
...
const ConsegneGestionale: React.FC = () => {
  ...
  useEffect(() => {
    ...
    api.get<Partial<DeliveryConfig>>(DELIVERY_ENDPOINT)
      .then((data) => {
        if (!alive) return;
        setCfg({
          weekdays:
          ...
          minLeadDays:
            typeof data.minLeadDays === "number"
              ? data.minLeadDays
              : DEFAULT_CFG.minLeadDays,
      ...
      .catch((err) => {
        if (!alive) return;
        console.error(err);
        setError(err instanceof ApiError ? err.message : "Impossibile leggere la configurazione.");
        setLoaded(true);
      });
