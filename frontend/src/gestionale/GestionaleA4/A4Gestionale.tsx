...
const A4Gestionale: React.FC = () => {
  ...
  // ======== Backend: config costi A4 ========
  useEffect(() => {
    let alive = true;

    (async () => {
      try {
        const data = await api.get<A4CostsResponse>("/api/admin/config/a4-costs");
        if (!alive) return;
        ...
        const i = data?.interni ?? ({} as Partial<Costi>);
        setInterni({
        ...
        console.error("Errore caricamento costi A4:", err);
        if (!alive) return;
  ...
  useEffect(() => {
    ...
    const fetchOrdini = async () => {
      try {
        const data = await api.get<OrderResponse[]>("/api/orders/a4");
        if (!alive) return;
        ...
        console.error("Errore caricamento ordini A4:", err);
      }
    };
  ...
  const handleSave = async () => {
    try {
      ...
      await api.put<A4CostsResponse>("/api/admin/config/a4-costs", {
        ...costi,
        interni: { ...interni },
