...
const Banner: React.FC = () => {
  ...
  useEffect(() => {
    let alive = true;
    const load = () => {
      api.get<BannerData>("/api/public/banner", { auth: false })
        .then((d) => { if (alive) setData(d); })
        .catch(() => { if (alive) setData(null); });
    };
    load();
    // niente websocket per un banner: un refresh periodico e' sufficiente per riflettere le modifiche dell'admin
