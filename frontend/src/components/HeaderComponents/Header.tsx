...
import { auth, onAuthStateChanged, logout, refreshCurrentUser, type CurrentUser } from "../../backend/auth";
...
const Header: React.FC = () => {
  const [user, setUser] = useState<CurrentUser | null>(auth.currentUser);
  const [accessLoading, setAccessLoading] = useState(!!auth.currentUser);
  ...
  const accessiblePages = useMemo(() => user?.pageAccess || [], [user]);
  ...
  useEffect(() => {
    ...
    // il ruolo/permessi potrebbero essere cambiati da un admin: ricarichiamo il profilo dal server
    // (il JWT in memoria potrebbe avere permessi non piu' aggiornati).
    if (auth.currentUser) refreshCurrentUser();
