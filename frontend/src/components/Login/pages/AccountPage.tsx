...
import { auth, recoverPassword, updateProfile } from "../../../backend/auth";
...
const AccountPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const rawOrders = await api.get<any[]>("/api/orders/mine");

        const userOrders: Order[] = rawOrders.map((data) => ({
          ...data,
          _tsMillis: data.timestamp ? Date.parse(data.timestamp) || 0 : 0,
        }));
  ...
  const handlePasswordReset = async () => {
    ...
    const user = auth.currentUser;
    if (!user?.email || !user?.telefono) return;
                      ...
                      <Checkbox
                        ...
                        onChange={async (e) => {
                          ...
                          if (!checked) {
                            ...
                            try {
                              await updateProfile({ corsoLaurea: "", annoAccademico: "" });
                            } catch { }
                          }
                        }}
                      />
