...
const FinalModalOverlay : React.FC<finalModalOverlayProps> = ({onConfirm, loading}) => {

    return <div className={classes["container"]}>
        <header>
            {loading === "submitted" ?  <h1>Caricamento avvenuto con successo</h1> : loading === "submitting" ? <h1>Caricamento in corso</h1> : <h1>Errore</h1>}
        </header>
        <div className={classes["content"]}>
            {loading === "submitted" ? <p>Sarai contatto presto da un nostro operatore.</p> : 
            loading === "submitting" ? 
                <div className={classes["spinner-container"]}>
                    <FontAwesomeIcon icon={faSpinner} className={classes["spinner"]} />
                    <p>Attendi fino al completo caricamento del file.</p>
                </div> : <p>Si è verificato un errore durante il caricamento del file. Riprova.</p>}
        </div>
        <footer>
...
const FinalModal : React.FC<finalModalOverlayProps>= ({onConfirm,loading}) => {
